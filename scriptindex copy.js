...
function calculateHP(base, ev = 0, iv = FIXED_IV, level = FIXED_LEVEL) {
  return Math.floor(((2 * base + iv + Math.floor(ev / 4)) * level) / 100) + level + 10;
}

function calculateOtherStat(base, ev = 0, nature = 'Serious', statKey, iv = FIXED_IV, level = FIXED_LEVEL) {
...
function clampEV(value) {
  ...
  return Math.min(252, Math.floor(number));
}
...
function buildPokemonPreview(pokemonName) {
  ...
  const iconsHtml = types.map((type, index) => {
    ...
    return icon
      ? `<img class="pokemon-type-icon" src="${escapeAttribute(icon)}" alt="${escapeAttribute(type)}" title="${escapeAttribute(type)}">`
      : '';
  }).join('');
  ...
  return `
    <div class="pokemon-preview-inline">
      <img
        ...
        src="${escapeAttribute(pokemon.sprite || '')}"
...
function renderTeamBuilder(team) {
  ...
  teamBuilder.innerHTML = `
    <div class="builder-layout box-style-layout">
...
function buildSimplePokemonSummary(pokemon) {
  ...
  const typesHtml = types.map((type, index) => `
    <div class="box-summary-type">
      ${typeIcons[index] ? `<img src="${escapeAttribute(typeIcons[index])}" alt="${escapeAttribute(type)}">` : ''}
    ...
    <div class="box-summary-card">
      <div class="box-summary-head">
        <img
          ...
          src="${escapeAttribute(dexPokemon.sprite || '')}"
      ...
      <details class="box-edit-details">
        ...
        <div class="box-edit-grid">
          <div class="field">
            ...
            <input
              type="text"
              data-field="item"
              value="${escapeAttribute(pokemon.item)}"
              placeholder="Ex: Focus Sash"
              maxlength="30"
            />
          </div>
        ...
        <div class="box-set-two-columns">
          ...
          <div class="box-ev-column">
            ...
            <div class="ev-total compact-ev-total" data-ev-total>
              <span>Total EVs: <strong>${getPokemonEVTotal(pokemon)}</strong> / 510</span>
            </div>
          </div>
        </div>
      </details>
  ...
  const typeHtml = types.map((type, index) => {
    ...
    return `
      <div class="summary-type">
        ${icon ? `<img src="${escapeAttribute(icon)}" alt="${escapeAttribute(type)}">` : ''}
        <span>${escapeHtml(type)}</span>
      </div>
    `;
  }).join('');
...
function handlePokemonInput(container, pokemonIndex, event) {
  ...
  if (field === 'item') {
    pokemon.item = target.value.trim();
  }
...
function updateEVDisplay(card, pokemon) {
  ...
  display.classList.toggle('over-limit', total > 510);
  display.innerHTML = `
    <span>Total EVs: <strong>${total}</strong> / 510</span>
    <span>${total > 510 ? 'Distribución inválida' : 'Distribución válida'}</span>
  `;
}
...
function validateDraft() {
  for (let i = 0; i < currentDraft.pokemon.length; i++) {
    ...
    const totalEVs = getPokemonEVTotal(pokemon);

    if (totalEVs > 510) {
      showToast(`El Pokémon ${i + 1} supera el máximo de 510 EVs.`);
      return false;
    }
  }
